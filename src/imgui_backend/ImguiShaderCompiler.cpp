...
//    Logger::log("Allocating ptr with size: %x aligned by %x\n", size, alignment);
  return Mem::AllocateAlign(alignment, ALIGN_UP(size, alignment));
}
...
bool ImguiShaderCompiler::CheckIsValidVersion(nvn::Device *device) {
  ...
  if (glslcDll->IsInitialized()) {
    ...
    if ((versionInfo.apiMajor >= static_cast<uint32_t>(minMajorVersion) &&
         versionInfo.apiMajor <= static_cast<uint32_t>(maxMajorVersion)) &&
        (versionInfo.apiMinor >= static_cast<uint32_t>(minMinorVersion) &&
         versionInfo.apiMinor <= static_cast<uint32_t>(maxMinorVersion))) {
